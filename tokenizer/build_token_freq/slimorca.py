...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build token frequency dictionary from SlimOrca.")
    parser.add_argument(
        "--tokenizer",
    ...
    parser.add_argument(
        "--output",
        default="slimorca_tok_dict.json",
...
def get_token_freq_dict(tokenizer, data, max_samples: int = 0):
    ...
    for sample in tqdm(iterator):
        for msg in sample['conversations']:
            tokenized = tokenizer.tokenize(msg['value'], add_special_tokens=False)
...
def main() -> None:
    ...
    slimorca_tok_dict = get_token_freq_dict(tokenizer, dataset[args.split], args.max_samples)
    with open(args.output, "w") as f:
        json.dump(slimorca_tok_dict, f, indent=4, ensure_ascii=False)
